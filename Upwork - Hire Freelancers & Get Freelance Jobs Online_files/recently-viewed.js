...
//# sourceMappingURL=angular.min.js.map
    ...
    (function(){
        var widget = document.getElementsByTagName('mc-recently-viewed');

        if (!widget.length) { // legacy version
            widget = document.getElementsByClassName('upwork-recently-viewed');
        }


        if (document.querySelectorAll('[ng-app]').length || document.querySelectorAll('[data-ng-app]').length ) {
            angular.element(document).ready(function () {
                angular.injector(['ng', 'marketing-widget.recently-viewed'])
                    .invoke(['$rootScope', '$compile', function($rootScope, $compile){
                        $compile(widget)($rootScope);
                        $rootScope.$digest();
                }]);
            });
        } else {
            ...
            angular.element(document).ready(function () {
                angular.bootstrap(widget, ['marketing-widget.recently-viewed']);
            });
        }
    })();

/**
 * Mock object for Applet
 */

var Applet = Applet || new function () {
        var basePath = '/';
        var phpVars = {};
        ...
        try {
            visitorId = document.cookie.match(/.*visitor_id=([0-9]+(?:\.[0-9]+){3}\.\d+).*/)[1];
        } catch (e) {
        }
...
(function() {
    ...
    /**
     * @ngdoc overview
     * @name components.core.model
     * @description
     *
     * The model core model module is responsible for providing basic
     * model functionality application-wide.
     */
    var model = angular.module('components.core.model', []);

    model.run(['eoModelRegistry', function(eoModelRegistry) {

        var initializing = false, fnTest = /xyz/.test(/* istanbul ignore next */ function () { }) ? /* istanbul ignore next */ /\b_super\b/ : /.*/;

        /**
         * @ngdoc interface
         * @name components.core.model.interface:Base
         * @description
         *
         * The Base model is not an Angular entity, it is an object that
         * provides rudimentary JavaScript inheritance based on this {@link http://ejohn.org/blog/simple-javascript-inheritance/#postcomment design pattern}
         * described by John Resig.  The Base model is registered with the
         * eoModelRegistry so that other modules and applications can take
         * advantage of the model building functionality it provides.
         *
         * # Super
         * A temporary method (._super) is created on all new model
         * definitions which is only accessible from within a sub-classes’
         * method, referencing the super-classes’ associated method.
         *
         * ```
         * var Person = Base.extend({
         *     name: null,
         *     init: function(obj) {
         *         //call the Base super method
         *         this._super(obj);
         *     }
         * }),
         * ```
         *
         * # Instances
         * The constructor of all new model definitions is set to the
         * name of the Model.  This provides access to the `instanceof`
         * operator.
         *
         * Checking instances should correctly compare the entire inheritance
         * chain of your Model, from Base all the way down to your model
         * definition.
         *
         * ```
         * var Person = Base.extend({
         ...
         * });
         *
         * joe instanceof Person && joe instanceof Base // => true
         * ```
         *
         * # Methods
         * - {@link components.core.model.interface:Base#methods_init Base.prototype.init}
         * - {@link components.core.model.interface:Base#methods_clone Base.prototype.clone}
         * - {@link components.core.model.interface:Base#methods_clean Base.prototype.clean}
         * - {@link components.core.model.interface:Base#methods_toObject Base.prototype.toObject}
         * - {@link components.core.model.interface:Base#methods_toJson Base.prototype.toJson}
         * - {@link components.core.model.interface:Base#methods_extendTo Base.prototype.extendTo}
         *
         */
        var Base = function () {};

        Base.extend = function BaseExtend (prop) {
            ...
            angular.forEach(prop, function (value, name) {
                prototype[name] = typeof prop[name] == "function" &&
                    typeof _super[name] == "function" && fnTest.test(prop[name]) ?
                    copyFn(name, prop[name]) : prop[name];
            });
         ...
         * @ngdoc method
         * @name init
         * @methodOf components.core.model.interface:Base
         * @param {Object=} obj An optional seed object literal from which to generate this model instance
         *
         * @description
         *
         * Parent initialization method to provide simple object creation.  This
         * method will accept a JavaScript object literal as a parameter and
         * attempt to build a specified model with the provided object.
         *
         * # Composition
         * Please note that this method currently does not support composition.  Any
         * composite models or collections of composite models will need to be handled
         * by the relevant model's initialization method.
         */
        Base.prototype.init = function(obj) {
            if (angular.isObject(obj)) {
                for (var propName in this) {
                    if (angular.isFunction(this[propName]) || obj[propName] === null) {
                        continue;
                    }
                    ...
                    if (propValue instanceof Array) {
                        ...
                        for (var i = 0; i < propValue.length; i++) {
                            this[propName].push(propValue[i]);
                        }
                    } else if (angular.isUndefined(propValue) && Object.getPrototypeOf(this).hasOwnProperty(propName)) {
                        //if propvalue is undefined use default value if it exists
                        var value = Object.getPrototypeOf(this)[propName];
                        ...
                        //use property on init object first
         ...
         * @description
         *
         * Method to clone the current model.  The returned value will
         * be a deep cloned object with no references to model instances.
         */
         ...
         * @description
         *
         * Method to remove all null and undefined property values
         * from an arbitrary object.
         */
        Base.prototype.clean = function(obj) {
            for (var k in obj) {
                if (obj[k] === null || angular.isUndefined(obj[k])) {
                    delete obj[k];
         ...
         * @description
         *
         * Method to convert the model instance into a JavaScript object
         * literal.  This is a recursive method; all references to models
         * will be destroyed and replaced with a clean object.
         */
        Base.prototype.toObject = function() {
            ...
            for (var propName in this) {
                ...
                if (angular.isFunction(this[propName]) || propName === '_super') {
                    continue;
                }

                propValue = this[propName];

                if (angular.isDefined(propValue) && propValue !== null && angular.isDefined(propValue.toObject) && propValue.toObject !== null) {
                    ...
                    for (var i = 0; i < propValue.length; i++) {
                        if (propValue[i] !== null && angular.isDefined(propValue[i].toObject) && propValue[i].toObject !== null) {
                            obj[propName].push(propValue[i].toObject());
                        } else {
                            obj[propName].push(propValue[i]);
                        }
                    }
                } else {
         ...
         * @name toJson
         * @methodOf components.core.model.interface:Base
         * @param {Object=} context An optional context from which to serialize a model
         *
         * @description
         *
         * Method to serialize a model into a JSON string format.
         */
        Base.prototype.toJson = function(context) {
            var obj = (angular.isDefined(context) && typeof angular.isDefined(this[context])) ? this[context].toObject() : this.toObject();
            return angular.toJson(this.clean(obj));
        };
         ...
         * @description
         *
         * Method to serialize a model into a Form data format. Analog of jQuery [$.param](http://api.jquery.com/jquery.param/)
         */
        Base.prototype.toFormData = function() {
         ...
         * @name extendTo
         * @methodOf components.core.model.interface:Base
         * @param {Object} obj An target object for extension
         *
         * @description
         *
         * Method to decorate an existing JavaScript object with Base functionality.
         * This method should be used if an object with prototype behavior
         * already exists and should not be overwritten.
         */
        Base.prototype.extendTo = function(obj) {
            obj.prototype.clean = this.clean;
            obj.prototype.clone = this.clone;
            obj.prototype.toObject = this.toObject;
            obj.prototype.toJson = this.toJson;
        };

        eoModelRegistry.put('Base', Base);
    }]);

    /**
     * @ngdoc service
     * @name components.core.model.service:eoModelRegistry
     * @module components.core.model
     * @requires $cacheFactory
     *
     * @description
     *
     * The eoModelRegistry is responsible for maintaining a list of
     * all Models registered with the application.
     *
     * # Purpose
     * This registry is intended to maintain an absolute list of Models
     * that will be used within a particular applet.
     *
     * # $cacheFactory
     * The registry is persisted with the Angular {@link https://docs.angularjs.org/api/ng/service/$cacheFactory $cacheFactory service}
     * under the 'eoModels' namespace.  The eoModelRegistry
     * exposes and proxies the $cacheFactory CRUD operations.
     *
     * @example
        <example module="demo">
            <file name="init.js">
                angular.module('demo', ['components.core.model'])

                .run(['eoModelRegistry', function(eoModelRegistry) {
                    var Base = eoModelRegistry.get('Base'),

                    Car = Base.extend({
                        wheel: null,
                        break: null,
                        //overwritten initialization
                        init: function(obj) {
                            this._super(obj);
                        }
                    }),
                    ...
                    eoModelRegistry.put('Car', Car);
                    eoModelRegistry.put('Wheel', Wheel);
                }]);
            </file>
            <file name="controller.js">
                angular.module('demo').controller('demoController', ['$scope', 'eoModelRegistry', function($scope, eoModelRegistry) {
                    var Car = eoModelRegistry.get('Car'), Wheel = eoModelRegistry.get('Wheel');
     ...
     */
    model.factory('eoModelRegistry', ['$cacheFactory', function($cacheFactory) {

        var cache = $cacheFactory('eoModels');
        ...
        return {
            ...
            put: function(key, value) {
                if (angular.isDefined(cache.get(key))) {
                    throw new cache.get('Exception')({ message: 'A ' + key + ' model has already been registered.' });
                }
    ...
    /**
     * @ngdoc service
     * @name components.core.models.service:eoModelFactory
     * @module components.core.model
     * @requires components.core.model.service:eoModelRegistry
     *
     * @description
     *
     * The eoModelFactory is responsible for returning instances of
     * models that have been registered with the eoModelRegistry.
     *
     * @example
        <example module="demo">
            <file name="ModelFactory.js">
                angular.module('demo', ['components.core.model', 'components.core.log.models'])

                .controller('demoController', ['$scope', 'eoModelFactory', function($scope, eoModelFactory) {
                    //event defined by the components.core.log.models module
                    $scope.event = eoModelFactory.get('Event', {
                        type: 'signout',
                        location: '/somepage',
                        sublocation: '/somesection'
                    });
                }]);
            </file>
            <file name="demo.html">
     ...
     */
    model.factory('eoModelFactory', ['eoModelRegistry', function(eoModelRegistry) {
        return {
            get: function(type, obj) {
                if (angular.isUndefined(eoModelRegistry.get(type))) {
                    return false;
                }

                var Model = eoModelRegistry.get(type);
    ...
    model.run(['eoModelRegistry', function(eoModelRegistry) {
        ...
        /**
         * @ngdoc object
         * @name components.core.model.object:Exception
         * @module components.core.model
         *
         * @property {string=} message The error message
         * @property {string=} stack Stack of error
         * @property {function=} init Initialization method that overwrites the Base `init` method
         *
         * @description
         *
         * Object to throw exceptions. You will need {@link components.core.log} to automatically send errors to server.
         */
        ...
        eoModelRegistry.put('Exception', Exception);
    }]);
...
(function() {
    ...
    /**
     * @ngdoc overview
     * @name components.core.config
     * @description
     *
     * The config module is responsible for maintaining and providing
     * configuration functionality to components and applications.
     */
    var config = angular.module('components.core.config', ['ui.bootstrap']);

    config.run(['eoConfig', function(eoConfig) {
        if (typeof Applet !== 'undefined') {

            if (typeof Applet.getUser !== 'undefined' && Applet.getUser() !== null) {
                eoConfig.put('User', Applet.getUser());
            }

            if (typeof Applet.getOrganization !== 'undefined' && Applet.getOrganization() !== null) {
                eoConfig.put('Organization', Applet.getOrganization());
            }

            if (typeof Applet.getVisitorId !== 'undefined' && Applet.getVisitorId() !== null) {

                var visitor = {
                    getVisitorId: Applet.getVisitorId
                };

                if (typeof Applet.getVisitorApiKey !== 'undefined' && Applet.getVisitorApiKey() !== null) {
                    visitor.getVisitorApiKey = Applet.getVisitorApiKey;
                }

                eoConfig.put('Visitor', visitor);
            }

            if (typeof Applet.getVar('urchinId') !== 'undefined' && Applet.getVar('urchinId') !== null) {

                var eo = {
                    analytics: {
                        urchinId: Applet.getVar('urchinId')
                    }
                };
            ...
            eoConfig.put('CsrfTokenHeaderName', Applet.getVar('csrfTokenHeaderName'));
            eoConfig.put('CsrfTokenCookieName', Applet.getVar('csrfTokenCookieName'));
            eoConfig.put('ClientStatsDMetrics', Applet.getVar('clientStatsDMetrics'));
            eoConfig.put('smfAjax', Applet.getVar('smfAjax'));
            eoConfig.put('OAuth2ClientId', Applet.getVar('clientOAuth2ClientId'));
        }
    }]);

    /**
     * @ngdoc service
     * @name components.core.config.service:eoConfig
     * @module components.core.config
     *
     * @requires $cacheFactory
     *
     * @description
     *
     * Responsible for maintaining a hash of application and
     * component configurations.
     *
     * @example
     <example module="demo">
         <file name="eoConfig.js">
         angular.module('demo', ['components.core.config'])
             .controller('demoController', ['$scope', 'eoConfig', function($scope, eoConfig) {
                            $scope.user = eoConfig.get('User');
                            $scope.organization = eoConfig.get('Organization');
                        }]);
         </file>
         <file name="demo.html">
             <div ng-controller="demoController">
                 ...
                 <p>{{user.getRid()}}</p>
                 <h2>Organization:</h2>
                 <p>{{organization.getRid()}}</p>
             </div>
         </file>
     </example>
     */
    config.factory('eoConfig', ['$cacheFactory', function($cacheFactory) {
        return $cacheFactory('eoConfig');
    }]);

    //global config
    config.config(['paginationConfig', function(paginationConfig){
...
(function() {
    ...
    var services = angular.module('components.core.interceptors.services', [
        'components.core.model',
        'ngResource'
    ]);

    services.run(['eoModelRegistry', function(eoModelRegistry) {

        var Base = eoModelRegistry.get('Base');

        var IncrementalHeader = Base.extend({
            name: null,
            value: null,
            init: function(obj) {
            ...
            increment: function(response) {
                this.value++;
                response.config.headers[this.name] = this.value;
            },
            getValue: function() {
                return this.value;
            },
            getBackoff: function() {
                return Math.ceil(Math.exp(this.value)) * 1000;
            }
        });

        eoModelRegistry.put('IncrementalHeader', IncrementalHeader);
    }]);

    /**
     * @ngdoc service
     * @name components.core.interceptors.services.service:eoRequest
     * @module components.core.interceptors.services
     *
     * @description
     *
     * Helper service is responsible for resending of the
     * same request to a server and returns promise.notify
     * to client-side.
     *
     */
    services.factory('eoRequest', ['$rootScope', '$q', '$injector', '$timeout', function($rootScope, $q, $injector, $timeout) {
        return {
            resend: function(response, interval) {
...
(function() {
    ...
    /**
     * @ngdoc overview
     * @name components.core.interceptors.headers
     * @description
     *
     * This module is responsible for providing an $http
     * log interceptor to send auxiliary headers X-Odesk-User-Agent, X-Requested-With
     *
     */
    var interceptor = angular.module('components.core.interceptors.headers', [
        'components.core.interceptors.services',
        'components.core.config'
    ]);

    interceptor.config(['$httpProvider' ,function($httpProvider) {
        $httpProvider.interceptors.push('oHeadersInterceptor');
    }]);

    interceptor.constant('X-Requested-With', 'XMLHttpRequest');
    interceptor.constant('X-Odesk-User-Agent', 'oDesk LM');

    /**
     * @ngdoc service
     * @name components.core.interceptors.headers.service:oHeadersInterceptor
     * @module components.core.interceptors.headers
     *
     * @requires X-Odesk-User-Agent
     * @requires X-Requested-With
     *
     * @description
     *
     * The oHeadersInterceptor is an $http `request` interceptor which adds `X-Requested-With`
     * and `X-Odesk-User-Agent` to all XHR requests. To change value of the headers you
     * can override corresponding constant.
     *
     * # Usage
     * The oHeadersInterceptor is automatically added to the $http interceptors
     * collection. This functionality will will be present when using
     * the configuration module.
     *
     */
    interceptor.factory('oHeadersInterceptor', ['X-Requested-With', 'X-Odesk-User-Agent', 'eoConfig',
        function (XRequestedWith, XOdeskUserAgent, eoConfig) {
            return {
                request: function (config) {
                    if (config.headers) {
                        angular.extend(config.headers, {
                            'X-Odesk-User-Agent': XOdeskUserAgent,
                            'X-Requested-With': XRequestedWith
                        });
                    }
                    
                    config.xsrfHeaderName = eoConfig.get('CsrfTokenHeaderName');
                    config.xsrfCookieName = eoConfig.get('CsrfTokenCookieName');
...
(function() {
    ...
    /**
     * @ngdoc overview
     * @name components.core.log.models
     * @description
     *
     * This module encapsulates the log model definitions.  It
     * will automatically append the models to the eoModelRegistry.
     */
    var models = angular.module('components.core.log.models', [
        'components.core.model'
    ]);

    models.run(['eoModelRegistry', function(eoModelRegistry) {

        var Base = eoModelRegistry.get('Base');

        /**
         * @ngdoc object
         * @name components.core.log.models.object:Event
         * @module components.core.log.models
         *
         * @property {string} event The type of event
         * @property {string=} location The location of the event
         * @property {string=} sublocation The sub-location of the event
         * @property {Object=} timestamp A timestamp of when the event occurred
         * @property {Array=} data Data associated with the event
         * @property {string=} qt_allocation QT allocation associated with the event
         * @property {function=} init Initialization method that overwrites the Base `init` method
         *
         * @description
         *
        ...
        /**
         * @ngdoc object
         * @name components.core.log.models.object:Error
         * @module components.core.log.models
         *
         * @property {string=} module Module name where the error occurred
         * @property {string=} entity Entity name (name of e.g. directive, service, controller, etc) where the error occurred
         * @property {string=} message The error message
         * @property {string=} url The location of the error
         * @property {string=} file The JavaScript file where the error occurred
         * @property {string=} line The line at which the error occurred
         * @property {string=} col The column at which the error occurred
         * @property {string=} trace A trace for the error
         * @property {function=} init Initialization method that overwrites the Base `init` method
         *
         * @description
         *
         * Model representing a client side error.
         */
        ...
        /**
         * @ngdoc object
         * @name components.core.log.models.object:HttpError
         * @module components.core.log.models
         *
         * @property {object=} response in $http format {@link: https://docs.angularjs.org/api/ng/service/$http}
         *
         * @description
         *
         * Model representing a http error, the model is inherited from Error model and accepts http response,
         * parses it and creates HttpError.status property from response.status and initializes HttpError.message
        ...
        var HttpError = Error.extend({
            ...
            init: function(obj) {
                if (typeof obj !== 'undefined') {
                    if(obj.response && obj.response.hasOwnProperty('status')) {
                        obj.status = obj.response.status;
                    }
                    if(obj.response && obj.response.hasOwnProperty('statusText')) {
                        obj.message = obj.response.statusText;
                    }
                    this._super(obj);
                }
            }
        ...
        eoModelRegistry.put('Event', Event);
        eoModelRegistry.put('Error', Error);
        eoModelRegistry.put('HttpError', HttpError);
    }]);
}());
(function() {
    ...
    /**
     * @ngdoc overview
     * @name components.core.log
     * @description
     *
     * The log module is responsible for managing log and error
     * logging functionality.
     */
    var log = angular.module('components.core.log', [
        'ngResource',
        'components.core.log.models',
        'components.core.config',
        'components.core.interceptors.headers'
    ]);

    log.constant('LOG_EVENT_ENDPOINT', '/api/o2/v1/logging/:id.json');
    log.constant('LOG_ERROR_ENDPOINT', '/error/java-script-error');
    log.constant('LOG_MAX_ERRORS_PER_PAGE', 10);

    log.config(['$provide', function($provide) {
        $provide.decorator("$exceptionHandler", ['$window', '$log', 'LOG_MAX_ERRORS_PER_PAGE', function($window, $log, LOG_MAX_ERRORS_PER_PAGE) {
            var errorCount = 0;
            return function(exception, cause) {
                /* istanbul ignore else */
                if ($window.onerror && $window.onerror.constructor === Function) {
                    errorCount++;

                    // parsing params
                    var params = [];
                    params.push(exception.message);

                    if (exception.stack && /\((\w+\.\w+)\:(\d+)\:(\d+)\)/.test(exception.stack)) {
                        Array.prototype.push.apply(params, exception.stack.match(/\((\w+\.\w+)\:(\d+)\:(\d+)\)/).splice(1));
                    } else {
                        Array.prototype.push.apply(params, ['unknown', 'unknown', 'unknown']);
                    }
                    params.push(exception.stack);

                    if (errorCount < LOG_MAX_ERRORS_PER_PAGE) {
                        // call for onerror handler
                        $window.onerror.apply(this, params);
                    } else {
                        $log.warn('Too much errors on the page, stopping sending to endpoint');
                    }
    ...
    log.run(['$window', 'oLog', 'eoModelFactory', function($window, oLog, eoModelFactory) {
        if (!$window.onerror && !$window.Qunit) {
            $window.onerror = function(message, file, line, col, trace) {
                var error = eoModelFactory.get('Error', {
                    message: message,
                    file: file,
                    line: line,
                    col: col,
    ...
    /**
     * @ngdoc service
     * @name components.core.log.service:oLog
     * @module components.core.log
     *
     * @requires components.core.log.service:oLogServices
     * @requires components.core.model.service:eoModelRegistry
     *
     * @description
     *
     * The oLog service is responsible for interfacing with our common logging APIs.
     * This is an abstracted common interface for all of our logging and error logging
     * functionality throughout our applets.
     *
     * # Methods
     * - {@link components.core.log.service:oLog#methods_event oLog.event}
     * - {@link components.core.log.service:oLog#methods_error oLog.error}
     *
     * # Models
     * An `error` passed to the error method and an `event` passed
     * to the evnt method are models that have already been registered
     * with the eoModelRegistry.  You can review them {@link components.core.log.models here}.
     *
     * @example
        <example module="demo">
            <file name="log.js">
                angular.module('demo', ['components.core.log'])

                .controller('demoController', ['$scope', 'oLog', 'eoModelFactory', function($scope, oLog, eoModelFactory) {

                    $scope.response = null;
                    $scope.event = eoModelFactory.get('Event', {
                        event: 'testEvent',
                        location: '/api/components.core.log.service',
                        sublocation: ':oLog'
                    });

                    oLog.event($scope.event).then(function(response) {
     ...
     */
    log.factory('oLog', ['oLogServices', 'eoModelRegistry', 'eoConfig', '$q', function(oLogServices, eoModelRegistry, eoConfig, $q) {

        /**
         * @ngdoc method
         * @name event
         * @methodOf components.core.log.service:oLog
         *
         * @description Method to log a user event
         * @param {Object} eventModel An event model object
         * @param {string=} proxy An optional string representing a proxy URI
         * @returns {Object} send event promise
         */
        function event(eventModel) {
            ...
            var Event = eoModelRegistry.get('Event');
            var Exception = eoModelRegistry.get('Exception');

            //require an Event model
            if (!(eventModel instanceof Event)) {
                throw new Exception({ message: 'Logging an event requires a valid Event model' });
            }

            //config to identify user
            if ((typeof eoConfig.get('User') === 'undefined' || eoConfig.get('User') === null) &&
                (typeof eoConfig.get('Visitor') === 'undefined' || eoConfig.get('Visitor') === null)) {

                throw new Exception({ message: 'Unable to identify user' });
            }

            //set up $q and request configs
            var deferred = $q.defer();
            var params = {
                id: (eoConfig.get('User')) ? eoConfig.get('User').getNid() : eoConfig.get('Visitor').getVisitorId()
            };

            //the o2 API requires the payload in this format
            angular.extend(params, {
                data: eventModel.toJson()
            });

            oLogServices.setConfig(config);

            //log event or proxy log event
            if (typeof proxy !== 'undefined') {
                params.proxy = proxy;

                oLogServices.proxyEvent(params).success(function(response) {
                    deferred.resolve(response);
                    return (response) ? /* istanbul ignore next */ response : false;
                }).error(function(error) {
                    deferred.reject(error);
                    return error ? /* istanbul ignore next */ error : false;
                });
            } else {
                oLogServices.event(params).$promise.then(function(response) {
                    deferred.resolve(response);
                    return (response) ? response : /* istanbul ignore next */ false;
                }, function(error) {
                    deferred.reject(error);
                    return error ? error : /* istanbul ignore next */ false;
                });
            }
         ...
         * @methodOf components.core.log.service:oLog
         *
         * @description Method to log application and window errors
         * @param {Object} error An error model object
         */
        function error(errorModel) {

            var Error = eoModelRegistry.get('Error');

            //require an Error model
            if (!(errorModel instanceof Error) || !errorModel.file) {
                return;
            }

            //serializing payload appropriately
            var params = angular.toJson({
                data: errorModel.toJson()
            });

            oLogServices.error(null, params).$promise.then(function(response) {
                return (response) ? response : /* istanbul ignore next */ false;
            });
        }
    ...
    /**
     * @ngdoc service
     * @name components.core.log.service:oLogServices
     * @module components.core.log
     *
     * @requires $resource
     *
     * @description
     *
     * The oLogServices is responsible for abstracting API
     * requests through $resource.  It provides a cleaner interface
     * and manages API endpoints and tokenized parameters.
     *
     * Please see {@link components.core.log.service:oLog oLog} for
     * details on how to utilize logging functionality.
     */
    log.factory('oLogServices', ['$resource', '$http', 'LOG_EVENT_ENDPOINT', 'LOG_ERROR_ENDPOINT', 'eoConfig', function($resource, $http, LOG_EVENT_ENDPOINT, LOG_ERROR_ENDPOINT, eoConfig) {

        /**
         * @description
         *
         * Action methods returned by $resource can not mutate headers sent
         * in http requests.
         *
         * This transformation function will check the eoConfig for auth
         * users vs. visitors and will append appropriate headers to the
         * request.
         */

        function getHeaders (eventConfig) {
            var headers = {}, i;

            if (typeof eoConfig.get('User') === 'undefined' &&
                typeof eoConfig.get('Visitor') !== 'undefined' &&
                typeof eoConfig.get('Visitor').getVisitorApiKey !== 'undefined') {

                angular.extend(headers, {
                    'X_ODESK_VISITOR': eoConfig.get('Visitor').getVisitorApiKey()
                });
            }

            if (eventConfig && eventConfig.headers && angular.isArray(eventConfig.headers)) {
                for (i = 0; i < eventConfig.headers.length; i++) {
                    if (angular.isObject(eventConfig.headers[i])) {
                        angular.extend(headers, eventConfig.headers[i]);
                    }
                }
            }
            return headers;
        ...
        function createEventEndpoint () {
            return $resource(LOG_EVENT_ENDPOINT, {
                id: '@id'
            }, {
                post: {
                    method: 'POST',
        ...
        function createProxyEndpoint () {
            return function (params) {
                var proxy = params.proxy + '/id/' + params.id;
                return $http({
                    method: 'POST',
                    url: proxy,
                    data: params.data,
        ...
        function createErrorEndpoint () {
            return $resource(LOG_ERROR_ENDPOINT, {}, {
                post: {
                    method: 'POST'
                }
            });
        ...
        //methods
        var headers, event, error, proxy,

            // sets configurations for next calls to endpoints
            setConfig = function (config) {
                headers = getHeaders(config);
                event = createEventEndpoint();
                error = createErrorEndpoint();
                proxy = createProxyEndpoint();
            };
        ...
        var service = {
            setConfig: setConfig,
            event: function () { return event.post.apply(this, arguments); },
            error: function () { return error.post.apply(this, arguments); },
            proxyEvent: function () { return proxy.apply(this, arguments); },
        };
        return service;
    }]);
...
(function() {
    ...
    /**
     * @ngdoc overview
     * @name components.core.utils
     * @description
     *
     * Provides commonly used utils
     */
    var utils = angular.module('components.core.utils', [
        'ngSanitize'
    ]);

    utils.constant('LINK_REWRITE_DEFAULTS', {
        LINK_URL_REGEXP: /((ftp|https?):\/\/[A-Za-z0-9._%+-]+)\S*[^\s.;,(){}<>"]/,
        LINK_DOMAINS_WO_BASEURL: /odesk\.com|upwork\.com$/,
        TARGET: '_blank'
    });

    /**
     * @ngdoc filter
     * @name components.core.utils.filter:unique
     * @module components.core.utils
     *
     * @description
     * Filters our duplicate items from a given array by
     * checking the specified key.
     *
     * @param {string=} key The name of the attribute of each object to compare for uniqueness; if the key is empty, the entire object will be compared
     * @returns {Array} The filtered array
     *
     * @example
        <example module="demo">
            <file name="unique.js">
                angular.module('demo', ['components.core.utils'])
            ...
            <file name="unique.html">
                <div ng-controller="demoController">
                    ...
                    <pre>{{arr | unique:attr | json}}</pre>
                </div>
            </file>
        </example>
     */
    utils.filter('unique', ['$parse', function ($parse) {
    ...
    /**
     * @ngdoc filter
     * @name components.core.utils.filter:highlight
     * @module components.core.utils
     *
     * @description
     * Highlights a text block based on a search string
     *
     * @param {string} search The needle to search for
     * @param {boolean=} caseSensitive Optional flag to enforce case-sensitivity
     *
     * @example
        <example module="demo">
            <file name="highlight.js">
                ...
                .controller('demoController', ['$scope', function($scope) {
                    $scope.copy = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque sollicitudin dapibus dapibus. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Nullam posuere sagittis volutpat. Praesent placerat est lectus, nec feugiat ex cursus in. Integer in justo rhoncus, bibendum odio id, pulvinar dolor. Praesent ultrices, ligula vitae posuere dapibus, magna ante ullamcorper mi, vel suscipit nunc diam et ipsum. In faucibus tempor elit, non suscipit dolor bibendum quis. Ut mattis sodales risus, a aliquet velit maximus eget. Ut sed condimentum magna, id commodo mi. Donec semper enim ante, in ultrices enim mollis et.';
                }]);
            </file>
            <file name="highlight.html">
                <div ng-controller="demoController">
                    <p>
                        <input ng-model="search" />
                    </p>
                    <p ng-bind-html="copy | highlight:search:true"></p>
                </div>
            </file>
        </example>
     */
    ...
    /**
     * @ngdoc filter
     * @name components.core.utils.filter:linkRewrite
     * @module components.core.utils
     *
     * @description
     * Rewrites urls e.g. 'http://angular.com' to html anchor tags. Supported protocols: http, https, ftp.
     *
     * @param {string=} baseUrl baseUrl will be concatenated with obtained links
     * @param {string=} domainsWoBaseUrl Do not apply baseUrl to specified in domainsWoBaseUrl regexp. by default will be used {@link components.core.utils.constant:LINK_REWRITE_DEFAULTS.LINK_DOMAINS_WO_BASEURL}
     * @param {string=} target is attribute specifies a window or a frame where the linked document is loaded, default: {@link components.core.utils.constant:LINK_REWRITE_DEFAULTS.TARGET} (_blank)
     * @returns {string} Sanitized html
     *
     * @example
     <example module="demo">
            <file name="linkRewrite.js">
                angular.module('demo', ['components.core.utils'])

                .controller('demoController', ['$scope', function($scope) {
                    $scope.html = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. http://upwork.com Pellentesque sollicitudin dapibus dapibus. http://angular.js';
                }]);
            </file>
            <file name="linkRewrite.html">
                <div ng-controller="demoController">
                    <p ng-bind-html="html | linkRewrite:'/leaving?ref='"></p>
                    <p ng-bind-html="html | linkRewrite:'https://upwork.com/leaving?ref=':undefined:'_self'"></p>
                    <p>{{html | linkRewrite:'/leaving?ref=' }}</p>
                    <p>{{html | linkRewrite:'https://upwork.com/leaving?ref=':undefined:'_self' }}</p>
                </div>
            </file>
        </example>
     */
    utils.filter('linkRewrite', ['$sanitize', 'LINK_REWRITE_DEFAULTS', function($sanitize, LINK_REWRITE_DEFAULTS) {
        return function(text, baseUrl, domainsWoBaseUrl, target) {
            if (!text) {return text;}
            ...
            var match,
                ...
                url,
                i,
                domainsWoBaseUrl = angular.isDefined(domainsWoBaseUrl) ? domainsWoBaseUrl : LINK_REWRITE_DEFAULTS.LINK_DOMAINS_WO_BASEURL,
                target = angular.isDefined(target) ? target : LINK_REWRITE_DEFAULTS.TARGET;

            while ((match = text.match(LINK_REWRITE_DEFAULTS.LINK_URL_REGEXP))) {
                // We can not end in these as they are sometimes found at the end of the sentence
                url = match[0];
                i = match.index;
                addText(text.substr(0, i));
                addLink(url, match[0]);
                text = text.substring(i + match[0].length);
            }
            addText(text);
            ...
            function addLink(url, text) {
                html.push('<a href="');
                if (angular.isDefined(baseUrl) && !(domainsWoBaseUrl).test(url)) {
                    html.push(baseUrl);
                }
    ...
    /**
     * @ngdoc service
     * @name components.core.utils.service:eoUtils
     * @module components.core.utils
     *
     * @description
     *
     * The eoUtils is provides commonly used utilities
     * that are implemented in third party libraries
     * such as jQuery and Underscore.js
     *
     * @example
     <example module="demo">
         <file name="eoBlock.html">
            <div ng-controller="demoController">
                ...
                <div class="visualize-results">
                    <p>Result of <strong>eoUtils.inArray({{arr}}, {{searchedFor}})</strong> is <strong>{{inArrayResult}}</strong></p>
                    <p>Selected inputs: {{inputs | json}}</p>
                    <p>Selected first label: {{firstLabel | json}}</p>
                </div>
            </div>
         </file>
         <file name="eoUtils.js">
             angular.module('demo', ['components.core.utils'])

             .controller('demoController', ['$scope', 'eoUtils', function($scope, eoUtils) {
                $scope.arr = [1, 2, 3, 4, 5];
                $scope.searchedFor = 3;
                $scope.inArrayResult = eoUtils.inArray($scope.arr, $scope.searchedFor);

                $scope.inputs = eoUtils.queryAll('.form-wrapper input');
                $scope.firstLabel = eoUtils.queryFirst('.form-wrapper label');
             }]);
         </file>
     </example>
     */
    utils.factory('eoUtils', [function() {

        /**
         * @ngdoc method
         * @name queryFirst
         * @methodOf components.core.utils.service:eoUtils
         *
         * @description Search the DOM tree by CSS selector for the first matched element
         *
         * @param {Object|String} el A DOM Element object or a string representing a valid CSS selector
         * @param {String=} selectors A string representing a valid CSS selector
         *
         * @returns {Object|null} An angular element or null on failure
         */
        function queryFirst(el, selectors) {
            return _query(el, selectors, false);
        }
         ...
         * @methodOf components.core.utils.service:eoUtils
         *
         * @description Search the DOM tree by CSS selector for all matched elements
         *
         * @param {Object|String} el A DOM Element object or a string representing a valid CSS selector
         * @param {String=} selectors A string representing a valid CSS selector
         *
         * @returns {Object|null} An angular element or null on failure
         */
        function queryAll(el, selectors) {
            return _query(el, selectors, true);
        }

        /**
         * @description DOM query (private generic method)
         *
         * @param {Object|String} el A DOM Element object or a string representing a valid CSS selector
         * @param {String=|null} selectors An optional string representing a valid CSS selector or null
         * @param {Boolean=} all A boolean that specifies whether all elements matched should be returned; the first element matched is returned otherwise
         *
         * @returns {Object|null} An angular element or null on failure
         * @private
         */
        function _query(el, selectors, all) {
            var method = all ? 'querySelectorAll' : 'querySelector';

            // default situation: we start searching from document top
            ...
            if (!el ||
                !(el === document || angular.isElement(el)) ||
                !(method in el) ||
                !angular.isString(selectors)) {return null;}

            return angular.element(el[method](selectors));
        }
         ...
         * @methodOf components.core.utils.service:eoUtils
         *
         * @description Searches an array for a given element
         *
         * @param {Array} arr A JavaScript array
         * @param {String|Number} el Element to locate in the array
         *
         * @returns {Boolean} true if the element is present, false if the element is not present
         */
        function inArray(arr, el) {
            return el && angular.isArray(arr) && arr.indexOf(el) > -1 ? true : false;
        }
         ...
         * @methodOf components.core.utils.service:eoUtils
         *
         * @description Appends URL with encoded query parameters
         *
         * @param {String} baseUrl Source url
         * @param {Object} params Object with query parameters
         * @returns {Object} Result url
         */
        function buildQueryUrl(baseUrl, params) {
            ...
            for (name in params) {
                if (params.hasOwnProperty(name)) {
                    url +=  separator + name + '=' + encodeURIComponent(params[name]);
                    separator = '&';
                }
            }
            return url;
         ...
         * @methodOf components.core.utils.service:eoUtils
         *
         * @description Parses a given URL or the document into pieces
         *
         * @param {String} url An optional URL to parse
         * @returns {Object} an object with properties representing each piece of the URL
         */
        function parseUrl(url) {

            url = url || document.location.href;

            var keys = ['source', 'protocol', 'authority', 'userInfo', 'user', 'password', 'host', 'port', 'relative', 'path', 'directory', 'file', 'queryString', 'anchor'],
                parser = (/^(?:(?![^:@]+:[^:@\/]*@)([^:\/?#.]+):)?(?:\/\/)?((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:\/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#\/]*\.[^?#\/.]+(?:[?#]|$)))*\/?)?([^?#\/]*))(?:\?([^#]*))?(?:#(.*))?)/),
                result = parser.exec(url),
                uri = {},
                pieces = 14;

            while (pieces--) {
                uri[keys[pieces]] = result[pieces] || '';
            }

            uri.query = uri.queryString;
            return uri;
        }
    ...
    /**
     * @ngdoc directive
     * @name components.core.utils.directive:eoHalt
     * @module components.core.utils
     * @element A
     *
     * @description
     *
     * Responsible for blocking browser events until
     * a provided callback is executed and returns a
     * promise.
     *
     * # Promise
     * This directive expects the eoHalt callback to
     * return a promise.  If a promise is not returned
     * this directive will cancel the original event
     * and fail to replay that event.
     *
     * # Scope
     * The anchors that utilize this directive should
     * not have an isolated scope;  if this is a
     * feature you need, please contact the UI team.
     *
     * @example
        <example module="demo">
            <file name="halt.html">
                <div ng-controller="demoController">
                    <a href="http://www.google.com" eo-halt="processor($event)">Click Me!</a>
                    <hr />
                    <p>Processing Click (3 seconds): {{processing}}</p>
                </div>
            </file>
            <file name="halt.js">
                angular.module('demo', ['components.core.utils'])
                .controller('demoController', function($scope, $q, $timeout) {
                    ...
                    $scope.processor = function(event) {
                        ...
                        $timeout(function() {
                            if (true) {
                                deferred.resolve('Some Success');
                            } else {
                                deferred.reject('Some Failure');
                            }
                        }, 3000);
     ...
     */
    utils.directive('eoHalt', ['$compile', function($compile) {
        return {
            restrict: 'A',
            link: function($scope, $element, $attrs) {
                $element.on('click', function(event) {
                    ...
                    $scope.$apply(function() {
                        ...
                        //using $eval rather than $parse; this enforces a shared scope
                        var promise = $scope.$eval($attrs.eoHalt);
                        ...
                        //replay click when the promise is resolved
                        promise.finally(function() {

                            //recompile the element to re-instate default click behavior
                            var compiled = $compile($element[0])($scope);
                            compiled.unbind('click');

                            //fire the click
                            compiled[0].dispatchEvent(new MouseEvent('click'));
                        });
                    });
                });
    ...
    /**
     * @ngdoc directive
     * @name components.core.utils.directive:eoFocus
     * @module components.core.utils
     * @element A
     *
     * @description
     *
     * Focus on element when expression is truthy.
     *
     * @example
     <example module="demo">
     <file name="focus.html">
        <div ng-controller="demoController">
            <button class="btn btn-default" ng-click="focusInput()">Focus Input</button>
            Value of scope variable is: {{focus}}
            <input class="form-control" type="text" eo-focus="focus">
        </div>
     </file>
     <file name="focus.js">
         angular.module('demo', ['components.core.utils'])
     ...
     */
    utils.directive('eoFocus', ['$parse', function($parse){
        return {
            restrict: 'A',
            scope: false,
            link: function($scope, $element, $attrs){
                var scopeVar = $attrs['eoFocus'];
                var parsedScopeVar = $parse(scopeVar);

                $scope.$watch(scopeVar, function(newVal) {
                    if(newVal === true){
                        $element[0].focus();
                    }
                });

                if(!parsedScopeVar.literal && !parsedScopeVar.constant) {
                    $element.on('blur', function() {
                        $scope.$evalAsync(function(){
                            parsedScopeVar.assign($scope, false);
                        });
                    });
                }
    ...
    /**
     * @ngdoc directive
     * @name components.core.utils.directive:eoClickOut
     * @module components.core.utils
     * @element A
     * @param {boolean=} eoClickOutDisable checking boolean before execute eoClickOut handler
     *
     * @description
     *
     * Run expression/callback when user clicks outside of an element. Any click event originating from or within the element will not trigger the expression to be run.
     *
     * @example
     <example module="demo">
         <file name="clickOut.html">
            <div ng-controller="demoController">
                <button class="btn btn-default" eo-click-out="clickedOutside()">Click Outside of Me!</button>
                <p ng-show="showMsg">Clicked Outside</p>
            </div>
         </file>
         <file name="clickOut.js">
             angular.module('demo', ['components.core.utils'])
     ...
     */
    utils.directive('eoClickOut', ['$parse', '$document', function($parse, $document){
        return {
            restrict: 'A',
            compile: function eoClickOutCompileFn(tElement, tAttrs){

                var eoClickOutFn = $parse(tAttrs['eoClickOut'], /* interceptorFn */ null, /* expensiveChecks */ true);

                return function eoClickOutLinkFn(scope, element, attrs){

                    var documentClickEventHandlerFn = function(event){
                        if(!element[0].contains(event.target)){
                            scope.$apply(function(){
                                eoClickOutFn(scope, {$event: event});
                            });
                        }
                    };

                    scope.$watch(function(){
                        return scope.$eval(attrs['eoClickOutDisable']);
                    }, function(newVal){
                        if(newVal){
                            $document.off('click', documentClickEventHandlerFn);
                        }
                        else {
                            $document.on('click', documentClickEventHandlerFn);
                        }
                    });
...
(function () {
    ...
    var eventlog = angular.module('marketing.global.eventlog', ['components.core.log', 'components.core.model', 'ngCookies']);

    /**
     * @desc Logs events to EventLog
     * @example <div olog-trigger olog-name='impression' olog-location='homepage' olog-sublocation=""></div>
     * @example <olog-trigger olog-name='impression' olog-location='homepage' olog-sublocation=""></olog-trigger>
     */

    eventlog.directive('ologTrigger', ['oLog', 'eoModelFactory', '$cookies', function (oLog, eoModelFactory, cookies) {
        return {
            restrict: 'AE',
            link: function (scope, element, attrs) {
                if (attrs.ologLocation && attrs.ologName) {
                    var event = eoModelFactory.get('Event', {
                        type: attrs.ologName,
                        location: attrs.ologLocation,
                        sublocation: attrs.ologSublocation,
                        event: attrs.hasOwnProperty('ologName') ? attrs.ologName : '',
                        data: attrs.hasOwnProperty('ologData') ? JSON.parse(attrs.ologData) : []
                    });

                    if (attrs.ologName == 'impression') {
                        ...
                        event.data.push({
                                'vt_kw': queryParams['vt_kw'],
                                'vt_med': queryParams['vt_med'],
                                'vt_src': queryParams['vt_src'],
                                'vt_cmp': queryParams['vt_cmp'],
                                'vt_adg': queryParams['vt_adg'],
                                'recognized': cookies['recognized'] ? 1 : 0
                            }
                        );
                    }

                    if (typeof attrs.ologEvent === 'undefined') {
                        oLog.event(event);
                    }
                    else {
                        element.on(attrs.ologEvent, function () {
                            oLog.event(event);
                        })
                    }
                }
...
(function () {
    ...
    var gtm = angular.module('marketing.global.gtm', []);

    gtm.directive('gtmTrigger', ['gtmTracker', function (gtmTracker) {
        return {
            restrict: 'AE',
            link: function (scope, element, attrs) {

                var data = {},
                    gtmEvent = attrs.gtmEvent || null;

                if (attrs.gtmTrigger === undefined) {
                    data = angular.fromJson(attrs.gtmData || '{}');
                    gtmTracker.track(gtmEvent, data);
                } else {
                    switch (attrs.gtmTrigger) {
                    case 'enter':
                        element.on('keyup', function (e) {
                            if (e.keyCode == 13) {
                                data = angular.extend({
                                    'query': element.val(),
                                    'type':  attrs.placeholder || ''
                                }, angular.fromJson(attrs.gtmData || '{}'));
                        ...
                        break;
                    default:
                        element.on(attrs.gtmTrigger, function () {
                            data = angular.fromJson(attrs.gtmData || '{}');
                            gtmTracker.track(gtmEvent, data);
                        });
                    }
    ...
    gtm.factory('gtmTracker', ['$window', function ($window) {

        $window.dataLayer = $window.dataLayer || [];

        /**
         * Tracks an event to Google Tag Manager
         */
        function track(event, data) {
            data = data || {};
            var time = +new Date();

            $window.dataLayer.push(angular.extend({
                event : event,
                time: time,
                url: location.href
            }, data));
...
(function() {
    ...
    angular.module('cssManager', [])
        .service('cssManager', cssManager);
    ...
    function cssManager() {
        ...
        function load(css) {
            ...
            if (angular.isString(css)) {
                ...
                angular.forEach(css, function (path) {
                    loadFile(path);
                });
            }
        }
        ...
        function loadFile(path) {
            ...
            path = path.replace('MARKETING_' + 'WIDGET' + '_VERSION', version);
...
angular.module("marketing-widget.recently-viewed.templates", []).run(["$templateCache", function($templateCache) {$templateCache.put("recently-viewed-profile.html","<a ng-href=\"https://www.upwork.com{{ ::profile.link }}\" target=\"_blank\">\n  <div class=\"up76cAzcME\">\n    <img ng-src=\"{{ ::profile.photo }}\" />\n  </div>\n  <div class=\"upALiyBiog\">\n    $<span ng-bind=\"::profile.hourlyRate | number: 0 \"></span><span class=\"up76TE8TMy\">/</span><span class=\"up5dco7cKd\">hr</span>\n  </div>\n  <div ng-bind=\"::profile.name\" class=\"upbai9eira\"></div>\n  <div ng-bind=\"::profile.title\" class=\"upjnTMETB5\"></div>\n</a>");
$templateCache.put("recently-viewed.html","<div ng-if=\"thankYouVisible\" style=\"opacity: {{thankYouDisappearing ? 0 : 1}}\" class=\"upLKT4bTbB\">\n  Thanks for your feedback!\n</div>\n\n<div ng-show=\"recentlyViewed.length &amp;&amp; !closed\" class=\"upe9cddc8x\">\n  <div class=\"upALcyqcog\">\n    <div class=\"upXjigKidb\">\n      <div class=\"upEyT6AT5q\">\n        <div ng-show=\"popupVisible\" ng-style=\"popupStyle\" class=\"up5dcoqcKd\">\n          <ul>\n            <li>\n              <a class=\"upo6i7xiXa\">Why are you closing this?</a>\n            </li>\n\n            <li ng-if=\"!formOpened\" ng-click=\"setReason(\'I\\\'m a freelancer\', \'vs_fl_widget_close_freelancer\')\">\n              <a href=\"\">I\'m a freelancer</a>\n            </li>\n\n            <li ng-if=\"!formOpened\" ng-click=\"setReason(\'It\\\'s annoying\', \'vs_fl_widget_close_annoy\')\">\n              <a href=\"\">It\'s annoying</a>\n            </li>\n\n            <li ng-show=\"formOpened\" class=\"up8KTj8TEe\">\n              <div class=\"upKXcL6cxe\">\n                <div class=\"\">Other</div>\n                <form name=\"form\">\n                  <textarea name=\"reason\" ng-model=\"reason\" eo-focus=\"focused\" pattern=\"[a-zA-Z0-9\\.,:;\\-_ ]{10,150}\" placeholder=\"Enter the reason...\" required=\"\" style=\"border-color : {{ form.reason.$valid ? \'#37a000\' : \'#d50000\' }}\"></textarea>\n\n                  <a class=\"upMaixpizr upe9TdzT8x\" href=\"\" ng-click=\"formOpened = false; reason =\'\'; popupStyle = {}\">Cancel</a>\n                  <a class=\"upMaixpizr upgXcKqcrp\" href=\"\" ng-click=\"(form.reason.$dirty &amp;&amp; form.reason.$valid) ? setReason(reason, \'vs_fl_widget_close_other\') : false\" ng-disabled=\"form.reason.$invalid\">Submit</a>\n                </form>\n              </div>\n            </li>\n\n            <li ng-show=\"!formOpened\">\n              <a href=\"\" ng-click=\"formOpened = true; popupStyle = {marginTop: \'-290px\', height: \'250px\'}\">Other</a>\n            </li>\n          </ul>\n        </div>\n\n        <span class=\"uppKin8iEe upo6T5oTXa\" ng-click=\"popupVisible = !popupVisible\"></span>\n      </div>\n\n      <div ng-click=\"expanded = !expanded; popupVisible = false;\" class=\"upjnTMETB5\">Your Recommended and Recently Viewed\n        Freelancers\n      </div>\n    </div>\n  </div>\n  <div class=\"upALcyqcog\">\n    <div ng-show=\"expanded\" class=\"upR7c8ocGa\">\n      <div class=\"upqMiXrikg\">\n        <div ng-disabled=\"from == 0\" class=\"upR7T8LTGa\">\n          <span class=\"uppKin8iEe upEycpbc57\" ng-click=\"from = (from &lt; 4) ? 0 : from - 4; to = (to &gt; 4) ? to - 4 : 4;\"></span>\n        </div>\n        <div ng-disabled=\"to == 12\" class=\"upo6iqBiXa\">\n          <span class=\"uppKin8iEe upo6T79TXa\" ng-click=\" from = (from &lt; 8) ? from + 4 : 8; to = (to &lt; 12) ? to + 4 : 12;\"></span>\n        </div>\n        <ul class=\"upRjcRGcpd\">\n          <li ng-repeat=\"profile in recentlyViewed track by $index\" ng-if=\"$index &gt;= from &amp;&amp; $index &lt; to\" ng-click=\"trackClick(profile)\" class=\"upxgiBdiAq\">\n            <mc-recently-viewed-profile profile=\"profile\"></mc-recently-viewed-profile>\n          </li>\n        </ul>\n      </div>\n    </div>\n  </div>\n</div>");}]);
angular.module('marketing-widget.recently-viewed', [
    'marketing-widget.recently-viewed.templates',
    'cssManager',
    'ngCookies',
    'marketing.global.gtm',
    'marketing.global.eventlog',
...
function config() {
    ...
    function getStorageUrl() {
        switch (environment) {
            case 'dev'   :
                return 'https://stage.upwork.com/i/storage/recently-viewed';
            default      :
                return 'https://www.upwork.com/i/storage/recently-viewed';
        }
    }
    ...
    function getApi() {
        switch (environment) {
            case 'dev' :
                return 'https://widget.upwork.com/api/v1/recently-viewed.json';
            default      :
                return 'https://widget.upwork.com/api/v1/recently-viewed.json';
        }
    }
    ...
    function getCss() {
        switch (environment) {
            case 'dev'  :
                return 'http://localhost:4020/dist/recently-viewed/recently-viewed.css';
            default     :
                return '//components.elance-odesk.com/marketing-widget/1.1.9/dist/recently-viewed/recently-viewed.css';
        }
    }
    ...
    function getTheme(theme) {
        ...
        switch (environment) {
            case 'dev'  :
                return 'http://localhost:4020/dist/recently-viewed/themes/' + theme + '.css';
            default     :
                return 'https://components.elance-odesk.com/marketing-widget/1.1.9/dist/recently-viewed/themes/' + theme + '.css';
        }
    }
}
(function () {
    angular.module('marketing-widget.recently-viewed')
        .service('mcRecentlyViewedDataservice', mcRecentlyViewedDataservice)
        .directive('mcRecentlyViewed', mcRecentlyViewed)
        .directive('mcRecentlyViewedProfile', mcRecentlyViewedProfile);

    mcRecentlyViewedDataservice.$inject = ['$http', 'config'];
    mcRecentlyViewed.$inject = ['mcRecentlyViewedDataservice', 'config', 'cssManager', '$cookies', 'gtmTracker', 'oLog', 'eoModelFactory', '$timeout'];

    function mcRecentlyViewed(mcRecentlyViewedDataservice, config, cssManager, $cookies, gtm, oLog, eoModelFactory, $timeout) {
        ...
        function link(scope) {
            ...
            scope.thankYouVisible = false;
            scope.thankYouDisappearing = false;
            ...
            if (scope.showIfHidden && !localStorage.getItem(scope.showIfHidden)) {
                return ;
            }

            config.setEnv(scope.env);

            // load default CSS
            cssManager.load(config.getCss());

            // load theme CSS
            if (scope.theme) {
                angular.forEach(scope.theme.split(','), function (theme) {
                    cssManager.load(config.getTheme(theme));
                });
            }

            // get recently viewed freelancers
            var recentlyViewed = $cookies.get('freelancers-viewed');
            recentlyViewed = recentlyViewed ? angular.fromJson(recentlyViewed) : null;

            // get recent search
            var recentlySearched = getRecentlySearched();

            mcRecentlyViewedDataservice.getRecentlyViewed({
                'recently_viewed[]': recentlyViewed,
                recently_searched: recentlySearched
            }).then(function (recentlyViewed) {
                scope.recentlyViewed = recentlyViewed;
            });

            scope.setReason = function (reason, event) {
                mcRecentlyViewedDataservice.setClosingReason(reason);

                localStorage.setItem('recently-viewed', 1);

                scope.closed = true;

                // track events
                try {
                    gtm.track(event, {reason: reason});

                    var reasonEvent = eoModelFactory.get('Event', {
                        type: event,
                        location: 'recommended_fl_widget',
                        sublocation: 'visitor_home_fl_widget',
                        event: event,
                        data: [{reason: reason}]
                    });

                    oLog.event(reasonEvent);
                } catch (e) {}

                // Disappearing
                scope.thankYouVisible = true;

                $timeout(function(){
                    scope.thankYouDisappearing = true;
            ...
            scope.trackClick = function(profile) {
                // track events
                try {
                    gtm.track('vs_fl_widget_profile', profile);

                    var event = eoModelFactory.get('Event', {
                        type: 'vs_fl_widget_profile',
                        location: 'recommended_fl_widget',
                        sublocation: 'visitor_home_fl_widget',
                        event: 'vs_fl_widget_profile',
                        data: [profile]
                    });
            ...
            scope.$watch('formOpened', function(newValue) {
                if (newValue) {
                    $timeout(function(){
                        scope.focused = true;
                    });
            ...
            try {
                gtm.track('vs-fl-widget-showup');

                var event = eoModelFactory.get('Event', {
                    type: 'vs_fl_widget_showup',
                    location: 'recommended_fl_widget',
                    sublocation: 'visitor_home_fl_widget',
                    event: 'vs_fl_widget_showup',
                    data: []
                });
    ...
    function mcRecentlyViewedDataservice($http, config) {
        ...
        function getRecentlyViewed(params) {
            return $http.get(config.getApi(), {params: params}).then(function (response) {
                return response.data;
            });
        }

        function setClosingReason(reason) {
            return $http.post(config.getStorageUrl(), {value: reason}).then(function (response) {
                return response.data;
            });
        }
    }

    function mcRecentlyViewedProfile() {
        return {
            scope: {
                profile: '='
            },
            templateUrl: 'recently-viewed-profile.html',
            restrict: 'E'
        };
    }
    ...
    function getRecentlySearched() {
        ...
        for (var i = 0; i < keys.length; i++) {
            if (keys[i].substring(0, 2) === 'up') {
                var value = localStorage.getItem(keys[i]);

                if (value.indexOf('upwork.com/o/profiles/browse') > -1) {
